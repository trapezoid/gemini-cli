...
- `coreTools`: An array of tool names to add to the list of allowed tools. This list is merged with the global `coreTools` configuration (from `settings.json`) using a union operation. This allows an extension to enable tools that may not be in the global configuration.
- `excludeTools`: An array of tool names to exclude from the model. This is merged with the global `excludeTools` configuration. If a tool is in both `coreTools` and `excludeTools`, it will be excluded. You can also specify command-specific restrictions for tools that support it, like the `run_shell_command` tool. For example, `"excludeTools": ["run_shell_command(rm -rf)"]` will block the `rm -rf` command.
